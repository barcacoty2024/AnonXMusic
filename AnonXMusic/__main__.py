...
async def init():
    if (
        ...
        LOGGER(__name__).error("Assistant danfir client variables not defined, exiting...")
        exit()
    await sudo()
    try:
    ...
    await app.stop()
    await userbot.stop()
    LOGGER("DanfirXMusic").info("Stopping Great x Music Bot...")
