...
async def init():
    if (
        ...
        LOGGER(__name__).error("Assistant wildan client variables not defined, exiting...")
        exit()
    await sudo()
    try:
    ...
    await app.stop()
    await userbot.stop()
    LOGGER("WildanXMusic").info("Stopping Great x Music Bot...")
